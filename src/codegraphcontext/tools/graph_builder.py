...
class GraphBuilder:
    ...
    async def build_graph_from_path_async(
        ...
        try:
            ...
            # Shared context override: use the context-scoped ignore file when provided.
            shared_cgcignore = os.getenv("CGC_CONTEXT_CGCIGNORE")
            if shared_cgcignore:
                cgcignore_path = Path(shared_cgcignore)
                if not cgcignore_path.exists():
                    try:
                        cgcignore_path.parent.mkdir(parents=True, exist_ok=True)
                        cgcignore_content = "# Auto-generated by CodeGraphContext (shared context)\n"
                        ...
                        cgcignore_path.write_text(cgcignore_content)
                        info_logger(f"Created shared context .cgcignore at {cgcignore_path}")
                    except OSError as e:
                        warning_logger(f"Could not create shared .cgcignore at {cgcignore_path}: {e}")

            # Walk up looking for .cgcignore (unless shared context override is active)
            if not cgcignore_path:
                while True:
