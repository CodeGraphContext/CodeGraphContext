...
class GraphBuilder:
    ...
    def add_file_to_graph(self, file_data: Dict, repo_name: str, imports_map: dict):
        ...
        with self.driver.session() as session:
            try:
                ...
                relative_path = (
                    str(Path(file_path_str).relative_to(Path(repo_result["path"])))
                    if repo_result
                    else file_name
                )
            except ValueError:
            ...
            if repo_result:
                ...
                warning_logger(
                    f"Repository node not found for {file_data['repo_path']} during indexing of {file_name}. Using original path."
                )
            ...
            for imp in file_data.get("imports", []):
                ...
                if lang == "javascript":
                    ...
                    params["module_name"] = imp.get(
                        "name"
                    )  # Use 'name' from imp as module name
    ...
    def _create_function_calls(self, session, file_data: Dict, imports_map: dict):
        ...
        skip_external = (
            get_config_value("SKIP_EXTERNAL_RESOLUTION") or "false"
        ).lower() == "true"
    ...
    def _create_all_function_calls(self, all_file_data: list[Dict], imports_map: dict):
        ...
        with self.driver.session() as session:
            for idx, file_data in enumerate(all_file_data):
                debug_log(
                    f"Processing file {idx + 1}/{len(all_file_data)}: {file_data.get('path', 'unknown')}"
                )
    ...
    def _create_all_inheritance_links(
        ...
        with self.driver.session() as session:
            for file_data in all_file_data:
                # Handle C# separately
                if file_data.get("lang") == "c_sharp":
                    self._create_csharp_inheritance_and_interfaces(
