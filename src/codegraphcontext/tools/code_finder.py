...
class CodeFinder:
    ...
    def analyze_code_relationships(self, query_type: str, target: str, context: Optional[str] = None, repo_path: Optional[str] = None, depth: Optional[int] = None, graph_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            if query_type == "find_callers":
                ...
                results = self.find_all_callers(target, context, repo_path=repo_path, depth=effective_depth, graph_name=graph_name)
                return {
                ...
                results = self.find_all_callees(target, context, repo_path=repo_path, depth=effective_depth, graph_name=graph_name)
                return {
    ...
    def find_most_complex_functions_in_file(self, file_path: str, limit: int = 20, repo_path: Optional[str] = None, graph_name: Optional[str] = None) -> List[Dict]:
