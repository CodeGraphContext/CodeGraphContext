...
class PythonTreeSitterParser:
    ...
    def _find_imports(self, root_node):
        ...
        # Process captures in source order. The query engine (tree-sitter
        # QueryCursor) may return captures grouped by name rather than by
        # position, so without this sort the dedup-by-name below would keep a
        # nondeterministic occurrence when the same module is imported on
        # multiple lines (e.g. try/except import fallbacks), producing flaky
        # IMPORTS edges. Sorting by (row, col) makes it always keep the first.
        captures = sorted(
            execute_query(self.language, query_str, root_node),
            key=lambda nc: (nc[0].start_point[0], nc[0].start_point[1]),
        )
