...
CodeGraphContext (CGC) **0.4.6** is a **context-aware code intelligence engine** that connects your repository to AI tools through an exact **knowledge graph** (not embeddings-only). It runs primarily as an **MCP server** with a **CLI** for indexing and queries, and optional **visualization**.
