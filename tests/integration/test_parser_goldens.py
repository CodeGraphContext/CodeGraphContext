...
def load_and_normalize(nodes_path, edges_path, current_repo_root, bundle_repo_root=None):
    ...
    for edge in edges:
        ...
        # CALLS / HEURISTIC_CALLS edges are generated by heuristic resolver fallbacks whose
        ...
        if edge_type in {"CALLS", "HEURISTIC_CALLS"}:
            continue
        ...
        if from_key and to_key:
            ...
            for k, v in props.items():
                if k == "args_key":
                    continue
                if edge_type == "IMPORTS" and k in {"line_number", "lang"}:
                    continue
                if isinstance(v, str):
